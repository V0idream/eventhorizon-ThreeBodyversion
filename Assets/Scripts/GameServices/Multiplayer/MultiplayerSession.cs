...
namespace GameServices.Multiplayer
{
    ...
    public sealed class MultiplayerSession : IInitializable, ITickable, IDisposable
    {
        ...
        public async void Host()
        {
            Disconnect();
            IsHost = true;
            SetStatus("正在监听本地端口 8779…请在 SakuraFRP 创建 TCP 隧道");
            try
            {
                _listener = new TcpListener(IPAddress.Any, HostPort);
                _listener.Start(1);
            ...
            {
                SetStatus("主机启动失败：" + error.Message);
        ...
        {
            if (_preparedSent || _remoteFleet.Count == 0 || !_manifestReceived || _expectedTextures.Count != 0) return;
            _preparedSent = true;
            Send("prepared", "MultiTest");
            SetStatus("双方资源已就绪" + (IsHost ? "，准备开始" : "，等待主机开始"));
            TryStartHost();
        }

        private void TryStartHost()
        {
            if (!IsHost || !_preparedSent || !_peerPrepared) return;
            _peerPrepared = false;
            Send("start", "MultiTest");
            SetStatus("同步完成，正在进入联机战斗…");
            BattleReady?.Invoke();
        }
        ...
        private bool _preparedSent;
        private bool _peerPrepared;
